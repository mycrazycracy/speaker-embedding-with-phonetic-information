...
# separate archives.
#
# We specify the number of repeat of each speaker, rather than calculated from the 
# number of frames.
...
am_egs_dir=          # egs dir for am training
num_train_archives=  # the num of the training archives
...
if [ ! -z $am_egs_dir ] && [ ! -z $num_train_archives ]; then
  echo "am-egs-dir and num-train-archives are all non-empty."
  echo "Only one can be used."

  exit 1
fi

if [ -z $am_egs_dir ]; then
  if [ -z $num_train_archives ]; then
    echo "Cannot figure out the num of training archives"
    exit 1
  fi 
else
  am_frames_per_eg=$(cat $am_egs_dir/info/frames_per_eg)
  num_train_archives=$(ls $am_egs_dir/egs.*.ark | wc -l)
  num_train_archives=$[$num_train_archives*$am_frames_per_eg]
fi
...
if [ $stage -le 3 ]; then
  ...
  $cmd $dir/log/train_subset_create_examples.1.log \
    nnet3-xvector-get-egs --compress=$compress --num-pdfs=$num_speakers $temp/train_subset_ranges.1 \
