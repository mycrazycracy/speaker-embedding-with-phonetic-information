...
if [ $stage -le 0 ]; then
  ...
  # shared part
  relu-batchnorm-layer name=tdnn1_share dim=512 input=Append(input@-2,$am_node@-2,input@-1,$am_node@-1,input@0,$am_node@0,input@1,$am_node@1,input@2,$am_node@2)
  ...
  relu-batchnorm-layer name=tdnn5_xvec dim=512 input=tdnn4_xvec
...
if [ $stage -le 1 ]; then
  if [ -d $am_egs_dir ]; then
    echo "$am_egs_dir exists, skip egs generation"
  else
    ...
    echo "New left and right context: $am_left_context, $am_right_context"

    if [ ! -d $am_feat_dir ] || [ ! -d $am_ali_dir ]; then
      echo "am_feat_dir and am_ali_dir must be given in this mode"
      exit 1
    fi
    ...
    sid/nnet3_cvector/cvector/get_egs_am.sh --cmd "$cmd" \
      ...
      $am_feat_dir $am_ali_dir $am_egs_dir
  fi
fi
