...
if [ $# != 4 ]; then
  echo "Usage: $0 <fgmm-model> <data-speaker-id> <post-dir> <extractor-dir>"
  echo " e.g.: $0 exp/sup_ubm/final.ubm data/train exp/post_mal exp/extractor_male"
...
if ! [ $nj_full -eq $(cat $post_dir/num_jobs) ]; then
  echo "Num-jobs mismatch $nj_full versus $(cat $post_dir/num_jobs) in $post_dir"
  exit 1
fi
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    for j in $(seq $nj_full); do
      Args[$j]=`echo "ivector-extractor-acc-stats --num-threads=$num_threads --num-samples-for-weights=$num_samples_for_weights $dir/$x.ie '$feats' 'ark,s,cs:gunzip -c $post_dir/post.JOB.gz|' -|" | sed s/JOB/$j/g`
    done
