...
if len(sys.argv) != 4:
    print("{0}: sre10_dir wav_dir output_dir".format(sys.argv[0]))
    quit()
...
for line in fp_wav.readlines():
    ...
    utt = sid + "_sre-" + suffix.replace("-", "_")
    wav = tmp[-2]
    channel = "a" if tmp[-3] == "1" else "b"
    assert channel == utt.rsplit("_", 1)[1]
    wav = wav.rsplit(".",1)[0].split("/",6)[-1]
    wav = wav_dir + wav + "_" + channel + ".wav"
