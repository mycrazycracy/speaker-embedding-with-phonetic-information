  ...
  if $use_gpu; then
    for g in $(seq $nj); do
      $cmd --gpu 1 ${dir}/log/extract.$g.log \
        ...
        "$nnet" "`echo $feat | sed s/JOB/$g/g`" ark,scp:${dir}/xvector_$name.$g.ark,${dir}/xvector_$name.$g.scp || exit 1 &
    done
    wait
  else
    $cmd JOB=1:$nj ${dir}/log/extract.JOB.log \
      nnet3-xvector-compute --use-gpu=no --min-chunk-size=$min_chunk_size --chunk-size=$chunk_size \
      "$nnet" "$feat" ark,scp:${dir}/xvector_$name.JOB.ark,${dir}/xvector_$name.JOB.scp || exit 1;
  fi
fi
