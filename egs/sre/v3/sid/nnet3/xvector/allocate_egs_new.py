...
def get_random_utt(spkr, spk2utt, utt2len, min_length):
    this_utts = []
    for utt in spk2utt[spkr]:
        if utt2len[utt] > min_length:
    ...
    for archive_index in range(args.num_archives):
        ...
        for n in range(this_num_egs):
            if len(spkrs) == 0:
                ...
                break
            spkr = spkrs.pop()
            utt = get_random_utt(spkr, spk2utt, utt2len, length)
            if utt == None:
                print('no utt for %s in len %d' % (spkr, length))
                continue
            utt_len = utt2len[utt]
