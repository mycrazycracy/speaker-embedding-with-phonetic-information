...
if $remove_noise; then
  echo "Remove noise before decoding"
  feats="ark,s,cs:apply-cmvn-sliding --norm-vars=$norm_vars --center=$center --cmn-window=300 scp:$sdata/JOB/feats.scp ark:- | select-voiced-frames ark:- scp,s,cs:$sdata/JOB/vad.scp ark:- |"
else
  feats="ark,s,cs:apply-cmvn-sliding --norm-vars=$norm_vars --center=$center --cmn-window=300 scp:$sdata/JOB/feats.scp ark:- |"
fi
