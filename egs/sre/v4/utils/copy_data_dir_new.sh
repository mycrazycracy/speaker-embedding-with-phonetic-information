...
cat $srcdir/wav.scp | awk -v p=$utt_prefix -v s=$utt_suffix '{printf("%s %s%s%s\n", $1, p, $1, s);}' > $destdir/wav_map
...
if [ -f $srcdir/segments ]; then
  utils/apply_map.pl -f 1 $destdir/utt_map <$srcdir/segments >$destdir/segments.new
  utils/apply_map.pl -f 2 $destdir/wav_map <$destdir/segments.new > $destdir/segments
...
if [ -f $srcdir/reco2file_and_channel ]; then
  utils/apply_map.pl -f 1 $destdir/wav_map <$srcdir/reco2file_and_channel > $destdir/reco2file_and_channel
...
rm -f $destdir/spk_map $destdir/utt_map $destdir/wav_map $destdir/segments.new
