...
if [ $# -eq 0 ]; then
  echo "$0 [--calldata] <output-data> <fisher-dir-1> [<fisher-dir-2> ...]"
...
tmpdir=$data/local/data_fisher

rm -r $data/local/data_fisher/links 2>/dev/null
mkdir -p $data/local/data_fisher/links || exit 1;

for subdir in fe_03_data_sph fe_03_tran; do
  found_subdir=false
  for dir in $*; do
    if [ -d $dir/$subdir ]; then
      found_subdir=true
      ln -s $dir/$subdir $links
    fi
  done
...
if [ $stage -le 0 ]; then
  ...
  find $links/fe_03_tran/trans -name '*.txt'  > $tmpdir/transcripts.flist

  for dir in fe_03_data_sph; do
...
#7.38 8.78 A: an- so the topic is 
  ...
  perl -e ' 
   ...
   open(R, "|sort >$data/reco2file_and_channel") || die "Opening reco2file_and_channel";
   ...
   close(R); close(T) ' $tmpdir $data || exit 1;
fi
...
if [ $stage -le 2 ]; then
  ...
  cp $tmpdir/text.2 $data/text
  # create segments file and utt2spk file...
  ! cat $data/text | perl -ane 'm:([^-]+)-([AB])-(\S+): || die "Bad line $_;"; print "$1-$2-$3 $1-$2\n"; ' > $data/utt2spk  \
     && echo "Error producing utt2spk file" && exit 1;

  cat $data/text | perl -ane 'm:((\S+-[AB])-(\d+)-(\d+))\s: || die; $utt = $1; $reco = $2; $s = sprintf("%.2f", 0.01*$3);
...
if [ $stage -le 3 ]; then
  ...
  cat $tmpdir/sph.scp | awk -v sph2pipe=$sph2pipe '{printf("%s-A %s -f wav -p -c 1 %s |\n", $1, sph2pipe, $2); 
    ...
    sort -k1,1 -u  > $data/wav.scp || exit 1;
fi

if [ $stage -le 4 ]; then
  # get the spk2gender information.  This is not a standard part of our
  # file formats
  # The files "filetable2: fe_03_p2_sph1 fe_03_05852.sph ff
  cat $links/fe_03_tran/doc/fe_03_p1_filelist.tbl \
    $links/fe_03_tran/doc/fe_03_p2_filelist.tbl | \
  perl -ane 'm:^\S+ (\S+)\.sph ([fm])([fm]): || die "bad line $_;"; print "$1-A $2\n", "$1-B $3\n"; ' | \
   sort | uniq | utils/filter_scp.pl $data/spk2utt > $data/spk2gender

  if [ ! -s $data/spk2gender ]; then
    ...
    cat $links/fe_03_tran/doc/fe_03_p1_filelist.tbl  $links/fe_03_tran/doc/fe_03_p2_filelist.tbl  | \
...
if [ ! -z "$calldata" ]; then # fix speaker IDs
  echo "Fix speaker IDs"
  cat $links/fe_03_tran/doc/*calldata.tbl > $tmpdir/combined-calldata.tbl
  local/fisher_fix_speakerid.pl $tmpdir/combined-calldata.tbl $data
  utils/utt2spk_to_spk2utt.pl $data/utt2spk.new > $data/spk2utt.new
  # patch files
  for f in spk2utt utt2spk text segments spk2gender; do
    cp $data/$f $data/$f.old || exit 1;
    cp $data/$f.new $data/$f || exit 1;
  done
