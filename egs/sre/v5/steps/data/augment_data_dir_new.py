...
def WriteSegments(segments, filename):
    fp = open(filename, "w")
    for seg in segments:
        fp.write("{0} {1} {2}\n".format(seg, segments[seg][0], segments[seg][1]))
    fp.close()
...
def main():
    ...
    is_segments = os.path.isfile(input_dir + "/segments")
    if is_segments:
        with open(input_dir + "/segments", "r") as f:
            for line in f.readlines():
                ...
                new_rec = rec + "-" + args.utt_suffix
                utt2uniq[new_utt] = utt
                segments[new_utt] = [new_rec, info]
    ...
    for line in wav_scp_file:
        ...
        wav = " ".join(toks[1:])
        dur = reco2dur[rec]
        new_wav = AugmentWav(rec, wav, dur, fg_snrs, bg_snrs, fg_noise_utts,
            bg_noise_utts, noise_wavs, noise_reco2dur, args.fg_interval,
            num_bg_noises)
        new_rec = rec + "-" + args.utt_suffix
        new_rec2wav[new_rec] = new_wav
        if not is_segments:
            utt2uniq[new_rec] = rec

    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    WriteDictToFile(new_rec2wav, output_dir + "/wav.scp")
    WriteDictToFile(utt2uniq, output_dir + "/utt2uniq")
    ...
    if is_segments:
        WriteSegments(segments, output_dir + "/segments")
