...
def get_args():
    parser = argparse.ArgumentParser(description="Writes ranges.*, outputs.* and archive_chunk_lengths files "
                                 "in preparation for dumping egs for dvector training.",
                                 epilog="Called by sid/nnet3/xvector/get_egs_dvector.sh")
...
def get_random_utt(spkr, spk2utt, utt2len, min_length, max_length): 
    # randomly choose a utterance
    this_num_utts = len(spk2utt[spkr])
    i = random.randint(0, this_num_utts-1)
    utt = spk2utt[spkr][i]

    length = random_chunk_length(min_length, max_length)
    if length > utt2len[utt]:
        length = utt2len[utt]
    free_length = utt2len[utt] - length
    offset = random.randint(0, free_length)
    return (utt, offset, length)
    
def get_random_utt(spkr, spk2utt, utt2len, min_length, max_length):
    length = random_chunk_length(min_length, max_length)
    this_utts = []
    for utt in spk2utt[spkr]:
        if utt2len[utt] > length:
            this_utts.append(utt)
    this_num_utts = len(this_utts)
    if this_num_utts == 0:
        utt = random.choice(spk2utt[spkr])
        offset = 0
        length = utt2len[utt]
        return (utt, offset, length)
    ...
    i = random.randint(0, this_num_utts-1)
    utt = this_utts[i]
    offset = get_random_offset(utt2len[utt], length)
    return (utt, offset, length)
...
def main():
    ...
    # all_egs contains 2-tuples of the form (utt-id, offset, length)
    all_egs= []
    ...
    for archive_index in range(args.num_archives):
        ...
        total_length = 0
        while total_length < args.frames_per_iter:
            if len(spkrs) == 0:
                print("Ran out of speakers for archive {0}".format(archive_index + 1))
                break
            spkr = spkrs.pop()
            (utt, offset, length) = get_random_utt(spkr, spk2utt, utt2len, args.min_frames_per_chunk, args.max_frames_per_chunk)  # return 3-tuple (utt, offset, length) 
            total_length += length
    ...
    for job in range(args.num_jobs):
        this_ranges = []   # A tuple (utt, index, offset, length), `index` is the index in this job
        ...
        for i in range(0, this_num_archives):
            ...
            for (utt, offset, length) in all_egs[cur_archive]:
                this_ranges.append( (utt, i, offset, length) )
        ...
        for (utt, i, offset, length) in sorted(this_ranges):
            archive_index = this_archives_for_job[i]
            print("{0} {1} {2} {3} {4} {5}".format(utt,
                                            i,
                                            archive_index + 1,
                                            offset,
                                            length,
                                            utt2spk[utt]),
              file=f)
            if utt2spk[utt] in pdf2num:
                pdf2num[utt2spk[utt]] += 1
            else:
                pdf2num[utt2spk[utt]] = 1
        f.close()
    ...
    print("allocate_egs_dvector.py: finished generating " + prefix + "ranges.* and " + prefix + "outputs.* files")
