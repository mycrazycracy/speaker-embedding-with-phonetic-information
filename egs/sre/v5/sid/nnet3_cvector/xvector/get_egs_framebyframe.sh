...
if [ $# != 2 ]; then
  ...
  echo "  --min-frames-per-chunk <#frames;50>                 # The minimum number of frames per chunk that we dump"
  echo "  --max-frames-per-chunk <#frames;300>                # The maximum number of frames per chunk that we dump"
  echo "  --num-repeats <#repeats;1>                       # The (approximate) number of times the training"
  echo "                                                   # data is repeated in the egs"
  echo "  --frames-per-iter <#samples;10000000>            # Target number of frames per archive"
  ...
  echo "                                                   # the middle."
  echo "  srand, num-heldout-utts, left-context, right-context"
...
if [ $stage -le 1 ]; then
  ...
  $cmd $dir/log/allocate_examples_train.log \
    sid/nnet3/xvector/allocate_egs_new.py \
...
echo $min_frames_per_chunk > $dir/info/frames_per_eg
...
if [ $frames_per_eg < 0 ]; then
    frames_per_eg=$min_frames_per_chunk
fi
...
#      nnet3-xvector-get-egs-framebyframe --num-pdfs=$num_pdfs $egs_opts \
