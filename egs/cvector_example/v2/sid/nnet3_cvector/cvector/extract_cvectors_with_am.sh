  ...
  if $use_gpu; then
    for g in $(seq $nj); do
      $cmd --gpu 1 ${dir}/log/extract.$g.log \
        ...
        2 "$nnet" "`echo $feat | sed s/JOB/$g/g`" "`echo $feat_am | sed s/JOB/$g/g`" ark,scp:${dir}/xvector_$name.$g.ark,${dir}/xvector_$name.$g.scp || exit 1 &
    done
    wait
  else
    $cmd JOB=1:$nj ${dir}/log/extract.JOB.log \
      ...
      2 "$nnet" "$feat" "$feat_am" ark,scp:${dir}/xvector_$name.JOB.ark,${dir}/xvector_$name.JOB.scp || exit 1;
  fi
fi

if [ $stage -le 1 ]; then
  echo "$0: combining cvectors across jobs"
  for j in $(seq $nj); do cat $dir/xvector_$name.$j.scp; done >$dir/xvector_$name.scp || exit 1;
fi

if [ $stage -le 2 ]; then
  echo "$0: computing mean of cvectors for each speaker"
  $cmd $dir/log/speaker_mean.log \
    ivector-mean ark:$data/spk2utt scp:$dir/xvector_$name.scp \
    ark,scp:$dir/spk_xvector_$name.ark,$dir/spk_xvector_$name.scp ark,t:$dir/num_utts.ark || exit 1;
fi
