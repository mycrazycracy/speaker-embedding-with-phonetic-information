...
if [ $# != 3 ]; then
  echo "Usage: $0 [opts] <data> <embeddings> <egs-dir>"
  echo " e.g.: $0 data/train exp/embedding exp/xvector_a/egs"
  echo ""
...
for f in $data/utt2num_frames $data/feats.scp $embedding/feats.scp; do
  [ ! -f $f ] && echo "$0: expected file $f" && exit 1;
done


feat_dim=$(feat-to-dim scp:$data/feats.scp -) || exit 1
embedding_dim=$(feat-to-dim scp:$embedding/feats.scp -) || exit 1
...
echo $embedding_dim > $dir/info/embedding_dim
...
info="scp,s,cs:utils/filter_scp.pl $temp/ranges.JOB $embedding/feats.scp |"
train_subset_info="scp,s,cs:utils/filter_scp.pl $temp/train_subset_ranges.1 $embedding/feats.scp |"
valid_info="scp,s,cs:utils/filter_scp.pl $temp/valid_ranges.1 $embedding/feats.scp |"
...
if [ $stage -le 3 ]; then
  ...
  for g in $(seq $nj); do
    ...
    $cmd $dir/log/train_create_examples.$g.log \
      ...
      "`echo $feats | sed s/JOB/$g/g`" "`echo $info | sed s/JOB/$g/g`" $outputs || touch $dir/.error &
  done
  wait
  ...
  $cmd $dir/log/train_subset_create_examples.1.log \
    ...
    "$train_subset_feats" "$train_subset_info" $train_subset_outputs || touch $dir/.error &
  ...
  $cmd $dir/log/valid_create_examples.1.log \
    ...
    "$valid_feats" "$valid_info" $valid_outputs || touch $dir/.error &
  wait
  if [ -f $dir/.error ]; then
